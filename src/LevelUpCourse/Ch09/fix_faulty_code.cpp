...
// The application is supposed to ask the user for a string, and print the number of vowels
...
int vowel_count(const std::string str) {
    ...
    std::transform(lowercase_str.begin(), lowercase_str.end(), lowercase_str.begin(), std::tolower);
    int count = 0;
    for (int i = 0; i < lowercase_str.length(); i++) switch (lowercase_str[i]) {
            case 'a':
            case 'e':
            case 'i':
            case 'o':
